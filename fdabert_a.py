...
from utils_a import fl_partition, initialise, train, test, logger, load_data
import argparse
import json
import logging
import math
...
class FlowerClient(fl.client.NumPyClient):
    def __init__(self, cid: str, fed_dir_data: str, args, accelerator, model, tokenizer):
        self.cid = cid
        self.args = args
    ...
    def fit(self, parameters, config):
        ...
        # Train
        datalen = train(self.args, self.model, self.tokenizer, self.accelerator, raw_datasets, num_workers)
    ...
    def evaluate(self, parameters, config):
        ...
        raw_datasets = load_data(args, self.train_file, self.validation_file)
        # Evaluate
        loss, perplexity, datalen = test(self.args, self.model, self.tokenizer, raw_datasets, num_workers)
...
def get_evaluate_fn(
    testset, args, accelerator, model, tokenizer
    ...
    def evaluate(
        ...
        loss, perplexity, datalen = test(args, model, tokenizer, accelerator, raw_datasets, 7)
...
if __name__ == "__main__":
    ...
    accelerator, model, tokenizer = initialise(args)
    ...
    strategy = fl.server.strategy.FedAvg(
        ...
        evaluate_fn=get_evaluate_fn("data/test.txt", args, accelerator, model, tokenizer),  # centralised evaluation of global model
    )

    def client_fn(cid: str):
        # create a single client instance
        return FlowerClient(cid, fed_dir, args, accelerator, model, tokenizer)
