...
testdf = pd.read_csv(args.answer_path, sep="\t").dropna() 
preddf = pd.read_csv(args.output_path, sep="\t").dropna() 
...
'''
    ...
    p,r,f,s = sklearn.metrics.precision_recall_fscore_support(y_pred=preddf["prediction"], y_true=testdf["label"])
    results = dict()
